...
const RAPIDAPI_KEY = import.meta.env.VITE_RAPIDAPI_KEY || 'b529388dabmsh3ecad4f3dffa3afp13dbf7jsna02bd8a6a3c1';//b529388dabmsh3ecad4f3dffa3afp13dbf7jsna02bd8a6a3c1
...
async function safeGet(path, opts = {}) {
    try {
      ...
      console.log('[CricApi] GET' , path, 'status', response.status);
      return {data: response.data, rawResponse: response}
       
    } catch (err) {
        console.error('[cricApi] GET', path, err?.response?.status, err?.message);
