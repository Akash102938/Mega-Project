...
function LiveMatch({ onSelect }) {
  ...
  const formatInningsScore = (innings) => {
    ...
    const wkts = innings.wickets ?? innings.wkts ?? innings.wktsTaken ?? 0;
    const overs = innings.overs ?? innings.oversPlayed ?? innings.oversString ?? '';
    const score = runs != null ? `${runs}/${wkts}` : '';
  ...
  const formatTeamScore = (matchScore, teamKey) => {
    ...
    if (obj.runs != null) return `${obj.runs}/${obj.wickets ?? obj.wkts ?? 0}`;
    return '';
  };
  ...
  const extractFromPayload = (payload) => {
    ...
    for (const tm of typeMatches) {
      ...
      for (const sEntry of seriesMatches) {
        ...
        const matchesArr = saw?.matches || saw?.matchesList || sEntry?.matches || [];
        if (!Array.isArray(matchesArr) || matchesArr.length === 0) continue;
  ...
  async function fetchLive() {
    ...
    try {
      ...
      const mapped = deduped.map((m) => {
        const flagA = flagForTeamName(m.team1.name || '');
        const flagB = flagForTeamName(m.team2.name || '');
        return {
          id: m.matchId,
          teamA: { name: m.team1.name, score: m.score1, flag: flagA },
          teamB: { name: m.team2.name, score: m.score2, flag: flagB },
          status: m.status,
          venue: m.venue,
          time: m.time,
          raw: m.raw,
  ...
  function FlagAndLabel({ flagObj, fallbackLabel }) {
    ...
    if (currentSrc && !imgError) {
      return <img src={currentSrc} alt={label ? `${label} flag` : 'flag'} className={liveMatchStyles.flagImage} onError={handleImgError} />;
    }
    ...
    return (
      <div className={liveMatchStyles.initialsContainer} style={getGradientStyle(c1, c2)} aria-hidden>
    ...
    <div className={liveMatchStyles.container}>
      <div className={liveMatchStyles.headerContainer}>
        ...
        <div className="flex items-center gap-3">
          ...
          <button className={liveMatchStyles.refreshButton} disabled={loading} onClick={fetchLive}>
            {loading ? 'Refreshing...' : 'Refresh'}
          </button>
        </div>
      </div>

      {quotaMode && <div className={liveMatchStyles.quotaAlert}>Quota may be exceeded - showing cached/simple data.</div>}
        ...
        <div className={liveMatchStyles.matchesGrid}>
          {matches.map((m) => (
            <div
              ...
              onKeyDown={(e) => {
                if (e.key === 'Enter' || e.key === ' ') onSelect && onSelect(m.id);
              }}
            ...
            >
              <div className={liveMatchStyles.matchCardInner}>
                <div className={liveMatchStyles.matchHeader}>
                  ...
                  <div className={liveMatchStyles.matchTime}>{typeof m.time === 'string' ? m.time.split(',')[0] : ''}</div>
                </div>
